...
class MakeCode
{
	...
	private static function createApiFile($config)
	{
		...
		foreach ( $list as $data ) {
			...
			if ( !file_exists(EntityDir."/$data[apiFile]") ) {
				file_put_contents(EntityDir."/$data[apiFile]", $content);
			}
		}
	}
	...
	private static function createModelFile($config)
	{
		...
		foreach ( $list as $data ) {
			...
			if ( !self::findFile(ModelDir, $data['modelFile']) ) {
				file_put_contents(ModelDir."/$data[modelFile]", $content);
			}
		}
	}
